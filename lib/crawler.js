...
Crawler.prototype._buildHttpRequest = function _buildHTTPRequest (options) {
    ...
    if (ropts.proxies && ropts.proxies.length) {
        var number  = -1;
        do {
            number  = Math.floor(Math.random()*(ropts.proxies.length - 1))+1;
        } while(number < 0 && number > ropts.proxies.length - 1);
        ropts.proxy = ropts.proxies[number];
    }
    ...
    if (options.debug) {
        if(ropts.proxies && ropts.proxies.length) {
            console.log(ropts.method+' '+ropts.proxy+' '+ropts.uri+' ...');
        } else {
            console.log(ropts.method+' '+ropts.uri+' ...');
        }
    }
...
Crawler.prototype._onContent = function _onContent (error, options, response, fromCache) {
    ...
    if (isHTML && options.jQuery && options.method !== 'HEAD') {
        ...
        if(typeof options.preCompile != 'undefined'){
            response.body   = options.preCompile(response.body);
        }
